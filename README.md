...
- **Fill all** writes only empty fields by default. Enable **Replace existing values** to overwrite after reviewing the warnings. Responses beginning with `N/A` also mark the CyberPass answer control as `N/A` when that control is present.
