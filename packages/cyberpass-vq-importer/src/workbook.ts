...
import type { ImportOptions, ParseOptions, ResponseChoice, SheetLike, VqInput, VqRow, WorkbookLike } from './types.js';
...
function sheetRows(sheet: SheetLike): Map<number, Map<string, string>> {
  ...
  for (const [address, raw] of Object.entries(sheet)) {
    ...
    const cell = raw as { v?: unknown; w?: string };
    const rowNumber = Number(match[2]);
    let row = rows.get(rowNumber);
    if (!row) rows.set(rowNumber, row = new Map());
...
export function responseChoice(value: string): ResponseChoice {
...
export function parseVendorQuestionnaire(workbook: WorkbookLike, options: ParseOptions = {}): VqRow[] {
  const result = new Map<string, VqRow>();
  for (const sheetName of options.sheetNames ?? workbook.SheetNames) {
    const sheet = workbook.Sheets[sheetName];
    if (!sheet) throw new VqImportError('SHEET_NOT_FOUND', `Worksheet "${sheetName}" was not found.`);
    let columns: { requirement: string; response: string } | undefined;
    for (const [rowNumber, cells] of sheetRows(sheet)) {
      ...
      const item: VqRow = { requirementId, rawRequirementId, value, predictedResponse: responseChoice(value), sheetName, row: rowNumber, column: columns.requirement, responseColumn: columns.response };
      const previous = result.get(requirementId);
      if (previous) {
        if (options.duplicates === 'first') continue;
  ...
  if (!result.size) throw new VqImportError('NO_REQUIREMENTS', 'No requirement rows detected. Expected SR No. (or Requirement ID) and Vendor Response headers on the same row.');
  return [...result.values()];
}

/** Decode a local File/Blob, ArrayBuffer, or Uint8Array into plain VQ rows. */
export async function readVendorQuestionnaire(input: VqInput, options: ImportOptions = {}): Promise<VqRow[]> {
