...
function receiver(events: VqFileEvents) {
  let generation = 0;
  return {
    dispose() { generation++; },
    async receive(files: FileList | null) {
      const current = ++generation;
      if (!files?.length) return;
      try {
        if (files.length !== 1) throw new VqImportError('FILE_COUNT', 'Choose one VQ workbook at a time.');
        const file = files[0];
        const vq = await importVq(file, events.options);
        if (current === generation) await events.onImport(vq, file);
      } catch (error) {
        if (current === generation) events.onError(error instanceof Error ? error : new Error(String(error)));
      }
    }
  };
}
...
export function bindVqFileInput(input: HTMLInputElement, events: VqFileEvents): () => void {
  if (input.type !== 'file') throw new TypeError('bindVqFileInput requires an input with type="file".');
  const accept = input.accept;
  if (!accept) input.accept = VQ_FILE_ACCEPT;
...
export function bindVqDropzone(element: HTMLElement, events: VqFileEvents): () => void {
  const handler = receiver(events);
